...
`SetupEfCoreHook` is the static class holding all of the above. `AddDbContextWithHook` (×2), `AddHook`, and the internal `AddHookRunner` are declared as C# `extension(IServiceCollection services)` members; `UseHooks` and `DisableHooks` are ordinary extension methods on `DbContextOptionsBuilder`/`DbContext` respectively, declared outside that block. Either way, all five sit in namespace `DKNet.EfCore.Hooks` — **not** the ambient `Microsoft.Extensions.DependencyInjection` namespace — so `using DKNet.EfCore.Hooks;` is required to call `AddDbContextWithHook`/`AddHook`/`UseHooks`/`DisableHooks`.
