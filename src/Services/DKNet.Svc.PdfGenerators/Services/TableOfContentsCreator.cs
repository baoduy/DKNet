...
internal class TableOfContentsCreator
{
        ...
        EmbeddedResourceService embeddedResourceService)
    {
        ...
        if (options.PageNumberOptions != null) conversionEvents.TempPdfCreated += InternalReadPageNumbers;
    }
    ...
    private string InternalCreatedDenestingTags(int depth, int lastDepth)
    {
        ...
        for (var i = 0; i < difference; ++i) html += Nl + "</li>" + Nl + _closeListElement;
    ...
    {
        if (_options.PageNumberOptions == null) return link.ToHtml();

        if (_linkPages == null) return link.ToHtml(-1); // Placeholder
    ...
    private static IEnumerable<LinkWithPageNumber> InternalParsePageNumbersFromPdf(PdfDocument pdf, Link[] links)
    {
        ...
        foreach (var page in pdf.GetPages())
        {
            ...
            foreach (var annotation in annotations)
            {
                ...
                {
                    if (!string.Equals(link.LinkAddress, linkAddress, StringComparison.OrdinalIgnoreCase)) continue;
                    ...
                    if (linksToFind.Count == 0) return linkPages; // All links found
    ...
    private void InternalReadPageNumbers(object? _, PdfEventArgs e)
    {
        ...
        if (_links == null) throw new InvalidOperationException("Links have not been created yet.");
    ...
    private string InternalToHtml(string markdownContent)
    {
        ...
        // close open tags
        for (var i = 0; i <= lastDepth; ++i) tocBuilder.Append(Nl + "</li>" + Nl + _closeListElement);
