...
- **Composes with signed-in-user auditing** — when the application also registers an `ICurrentUserProvider`
  from [`DKNet.EfCore.AuditLogs`](../DKNet.EfCore.AuditLogs/README.md), this package supplies `OwnedBy` only and
  the audit identity comes from that provider. With no current-user provider registered it keeps filling
  `CreatedBy`/`UpdatedBy` from the ownership key exactly as before, so existing applications need no code change.
...
| `IDataOwnerProvider.GetOwnershipKey()` | your provider | required, no default | The owner key stamped on new `IOwnedBy` rows, and — unless an `ICurrentUserProvider` is registered — on `CreatedBy`/`UpdatedBy` too. Null or blank means the hook stamps nothing. |
...
[`DKNet.EfCore.AuditLogs`](../DKNet.EfCore.AuditLogs/README.md); that package documents the provider contract and
