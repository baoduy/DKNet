...
internal static class SequenceExtensions
{
    ...
    extension(ModelBuilder modelBuilder)
    {
        ...
        internal void RegisterSequences(params Assembly[] assemblies)
        {
            var sequenceEnums = assemblies.SelectMany(a => a.Extract().Enums()).ToList()
                .Where(e => e.HasAttribute<SqlSequenceAttribute>())
                .ToList();

            foreach (var type in sequenceEnums) modelBuilder.RegisterSequencesFromEnumType(type);
        }

        private void RegisterSequencesFromEnumType(Type enumType)
        {
            ...
            foreach (var f in fields)
            {
                ...
                if (fieldAtt.StartAt > 0) seq.StartsAt(fieldAtt.StartAt);

                if (fieldAtt.IncrementsBy > 0) seq.IncrementsBy(fieldAtt.IncrementsBy);

                if (fieldAtt.Min > 0) seq.HasMin(fieldAtt.Min);

                if (fieldAtt.Max > 0) seq.HasMax(fieldAtt.Max);
