...
> **Actioned 2026-09-03.** All 32 open items were worked through, one agent per package area, each in an isolated worktree with a zero-warning solution build and its own test projects green. **30 applied** (several in part — the item text says which), **2 withdrawn** as wrong on the merits: [P1](#p1) and [P13](#p13). Where a finding's prescribed fix was rejected in favour of a smaller or safer one, the reason is recorded in the item.
>
> **Deferred by decision, not by oversight:** the full expression-tree rewrite in [P4](#p4) (and therefore [S11](simplification.md#s11)); deleting the navigation-walk mechanism in [P22](#p22), though the spike now shows EF Core covers the common case; the `State = Modified` semantics in [P23](#p23); caching the `AesGcm` instance in [P28](#p28); the double-base64 wire format in [P29](#p29).
>
> **Not verified locally:** `AspCore.Idempotency.MsSqlStore.Tests` — SQL Server has no working container on ARM64. [P2](#p2)'s logic is shared with `NpgsqlStore` and is verified there against a real Postgres database; only the SQL-Server-specific unique-violation detection (error 2601/2627 vs SQLState 23505), which this change did not touch, is unexercised. Re-validate on the x64 runner before merging to `dev`.
...
**Withdrawn — not applied.** `ToPageEnumerable` is internal, but 11 tests in `EfCore.Specifications.Tests/Paging/PageAsyncEnumeratorTests.cs` assert the chunked, page-size-driven behaviour itself (`ArgumentOutOfRangeException` on an invalid page size, exact multi-page ordering, per-page cancellation). Collapsing to `AsAsyncEnumerable()` changes that observable contract, so this needs a maintainer decision rather than a mechanical fix.
...
**Interim version applied; full rewrite still deferred.** `ToPascalCase` and `ResolvePropertyType` are memoised in `ConcurrentDictionary` caches, and the double `ToPascalCase` call collapsed into one `TryNormalizePropertyName`. Together with the `UseParameterizedNamesInDynamicQuery` change this removes the reflection, most of the allocation, and the plan-cache pollution. The expression-tree rewrite that would delete `System.Linq.Dynamic.Core` from the typed path — and with it [S11](simplification.md#s11) — was deliberately not attempted.
...
**Spike run; option 2 applied, mechanism kept.** The spike test `SaveChangesAsync_NewChildAddedToTrackedParentCollection_IsInsertedWithoutCustomMechanism` confirms EF Core's own relationship fixup **already** inserts a new child added to a tracked parent's collection navigation, with no call into `AddNewEntitiesFromNavigations` — evidence that option 1 (delete the mechanism) may hold, but full removal is left as a maintainer decision. Option 2 is in: `Unchanged` dropped from the state filter, `entry.Collection(nav.Name).CurrentValue` replaces the reflection read, and `IsNewEntity` short-circuits on `Detached` (which also resolves a doc/code mismatch — the XML doc already promised that).
...
**Safe half applied.** The duplicated `_dbContext.Entry(entity)` call is hoisted into one local. The `State = Modified` semantic change is **not** done: it needs a decision on whether detached entities stay supported, and it changes `UpdateAsync`'s return contract. Still open.
...
**Applied, two sub-items skipped.** `IsKeyProcessedAsync` now reserves with one `StringSetAndGetAsync(..., When.NotExists)`; Redis returns the pre-existing value in the same round trip when NX blocks the write, so the collision path went 2–3 round trips → 1. Skipped: passing the sanitized key down between store methods (needs a breaking signature change on the public `IIdempotencyKeyStore`), and the `JsonSerializerContext`/AOT item (conflicts with `IdempotencyOptions.JsonSerializerOptions` being public and user-configurable).
...
**Applied, with one deviation.** `SetOwnedProperty` uses `entry.Property(name).CurrentValue` when `entry.Metadata.FindProperty(name)` resolves it. The finding said to delete `FindWritableProperty` outright; it was kept as a fallback because `AuditedNoUpdatedByColumnEntity` and `AuditedNoUpdatedOnColumnEntity` depend on reflection reaching EF-unmapped audited columns. Fail-closed throwing behaviour is preserved and is now covered by a real `SaveChangesAsync` test instead of a reflection-level one.
...
**Applied to `HookRunnerInterceptor` only.** Its `LogInformation` sites — including the one genuinely missing an `IsEnabled` guard — became `[LoggerMessage]` source-generated partials, which are allocation-free and need no guard. The other call sites the finding names sit in files owned by separate changes and were left alone.
...
**Applied in part.** A `string` fast path in `ConvertOrDefault` skips `TypeDescriptor` for the dominant case, and per-type `TypeConverter` / boxed-default caches replace the per-request `GetConverter` and `Activator.CreateInstance` calls. The compiled-setter-delegate half (M-effort in the finding) was left: the S-effort items already remove the two costs the finding calls out.
...
**Buffer copies applied; instance caching deliberately skipped.** Encrypt and Decrypt now slice one buffer with spans instead of `Buffer.BlockCopy` triples, and the iv‖tag‖ciphertext wire format is pinned byte-identical by a known-answer test. The per-call `AesGcm` construction stays: the provider is attached to EF Core's cached model and invoked concurrently across `DbContext` instances, and `AesGcm` is not documented thread-safe — a shared cached instance risks data corruption. The safe alternatives (ThreadLocal pool, lock) either leak native handles or serialize throughput, which is the benchmark caveat the finding itself raises.
...
**Applied except the wire-format item.** `RsaEncryption.PublicKey`/`PrivateKey` now cache their export (the underlying key material is immutable for the instance's lifetime). The `AesGcmEncryption` double-base64 packaging is **not** collapsed: it would change the stored ciphertext format and make existing ciphertexts undecryptable, which needs a migration path rather than an edit.
...
**Applied except one sub-item, and the `decimal.TryParse` suggestion is rejected.** `ExtractDigits` fills a single pooled buffer in one pass; `IsNumber` went from four LINQ passes to one loop with identical semantics; `PropertyExtensions.GetProperty` is now cached. The finding's suggestion to replace `IsNumber` with `decimal.TryParse` is wrong: existing tests assert `"123.456,789".IsNumber() == true` and `"123,456,789,012".IsNumber() == true`, so the hand-rolled rule is a permissive custom format, not decimal grammar — `TryParse` would fail those. `GetEnumInfos` was skipped: its cited bug was already fixed in `66a77ef`, and there are no production callers.
...
**Withdrawn — the prescribed fix would cause a regression.** `CrudGenerator` scans referenced assemblies because entities routinely live in a Domain assembly compiled separately from the API assembly generating against them. That is the tested, supported shape: `SlimBus.Generators.Tests/GeneratorTestHelper.cs:9-14` compiles each test's entity into a separate in-memory assembly on purpose, and `SlimBus.Generators.Tests.Domain` + `.Tests.Api` do the same at build time. `ForAttributeWithMetadataName` only observes attributes in the current compilation's syntax trees, so it cannot see `[CrudCreate]` on a type present only as referenced metadata. Adopting it would silently stop discovering those entities.
...
**Applied in part; two sub-problems do not reproduce.** `Target` already has a custom `Equals`/`GetHashCode` comparing symbols by `ToDisplayString` and hash sets by `SetEquals`, so the described cache-miss behaviour does not occur. The third sub-problem was real: `.Combine(context.CompilationProvider)` fed a `Compilation` parameter that was traced end-to-end and never read, forcing full DTO regeneration on every keystroke. Removed, along with the dead parameter through four methods. `RaisesEventValidator`'s own combine was left — its collision check genuinely needs whole-compilation lookups. Byte-identical output verified by building three real generator consumers with `EmitCompilerGeneratedFiles=true` before and after and diffing the trees.
